...
class Neuron:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,
        ...
        # Single loop allowed for training
        for i in range(iterations + 1):
            # 1. Forward propagation (skip on the absolute last step)
            if i < iterations:
                self.forward_prop(X)

            # 2. Handle verbose and graph data collection
            ...
            # 3. Gradient descent (skip on the absolute last step)
            if i < iterations:
