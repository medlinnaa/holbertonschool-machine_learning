...
class DeepNeuralNetwork:
    ...
    def forward_prop(self, X):
        ...
        """
        # Store input data as A0 in the cache
        self.__cache['A0'] = X

        # Single loop to move through all layers
        for layer in range(1, self.__L + 1):
            # Access weights and biases of current layer
            w = self.__weights['W' + str(layer)]
            b = self.__weights['b' + str(layer)]

            # Access the output of the previous layer
        ...
        # Return the final activated output and the cache
