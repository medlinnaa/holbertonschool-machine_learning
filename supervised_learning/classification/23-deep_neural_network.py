...
class DeepNeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05,
        ...
        # Single functional loop to train the network
        i = 0
        while i < iterations + 1:
            # 1. Forward propagation (skip on final iteration count)
            if i < iterations:
                a_last, _ = self.forward_prop(X)
            else:
                # Get prediction from existing cache on final step
                a_last = self.__cache['A' + str(self.__L)]

            # 2. Tracking data at 0, every step, and final iteration
            ...
            # 3. Apply gradient descent (skip on the last pass)
            if i < iterations:
