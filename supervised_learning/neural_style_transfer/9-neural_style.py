...
class NST:
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01,
        ...
        # Initialize the generated image starting as the content image
        generated_image = tf.Variable(self.content_image)

        best_cost = float('inf')
        best_image = None

        # Loop runs (iterations + 1) times to include the final evaluation
        for i in range(iterations + 1):
            ...
            if current_cost < best_cost:
                ...
                # Keep a copy of the best image found so far
            ...
            # Apply gradients, except on the very last evaluation step
            if i < iterations:
                optimizer.apply_gradients([(grads, generated_image)])
                # Clip the pixel values to maintain validity [0, 1]
