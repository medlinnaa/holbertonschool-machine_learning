...
def build_model(nx, layers, activations, lambtha, keep_prob):
    ...
    # This line means: Take 'inputs', pass it through 'Dense', call the result 'x'
    ...
    for i in range(1, len(layers)):
        ...
        # Pass the result of Dropout into the next Dense layer
        x = K.layers.Dense(layers[i], activation=activations[i], kernel_regularizer=reg)(x)

    # 4. Create the model by connecting the start (inputs) to the end (x)
